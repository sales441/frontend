...
export default function Ads() {
  ...
  const applyBids = async () => {
    try {
      await API.post("/ads/bids/apply", { adjustments: [{ keyword: "waxing kit", change: "+10%" }] });
              ...
              <ul>
                {campaigns.map((c) => (
                  <li key={c.id} className="mb-2 flex justify-between">
                    <span>{c.name}</span>
                    <button onClick={() => loadKeywords(c.id)} className="px-2 py-1 bg-gray-200 rounded">Ver keywords</button>
                  </li>
                ))}
            ...
            <div className="bg-white p-4 rounded shadow">
              ...
              {selected ? (
                <>
                  <ul>
                    {keywords.map((k) => (
                      <li key={k.id} className="mb-2 flex justify-between">
                        <span>{k.text} <span className="text-xs text-gray-500">CTR {k.ctr}% · ACOS {k.acos}%</span></span>
                        <span>${k.bids}</span>
                      </li>
                    ))}
                  ...
                  <button onClick={applyBids} className="mt-4 px-3 py-2 bg-yellow-500 rounded">Aplicar Ajustes</button>
                </>
              ) : (
